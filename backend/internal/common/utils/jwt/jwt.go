...
func NewJWTUtil(secretKey string, tokenExpire int64, issuer string, audience []string) JWTUtil {
	return &jwtUtil{
...
func (ju *jwtUtil) GenerateToken(role string) (string, error) {
	// 生成 JWT 令牌
	claims := CustomClaims{
		Role:      role,
		ExpiresAt: jwt.NewNumericDate(time.Now().Add(time.Duration(ju.tokenExpire) * time.Hour)),
	...
	tokenString, err := token.SignedString(ju.secretKey)
	if err != nil {
...
func (ju *jwtUtil) ParseToken(tokenString string) (*CustomClaims, error) {
	// 解析 JWT 令牌
	claims := &CustomClaims{}
